...
START_IMAGE_TOKEN = ""
SOS_POINTS_URL = "https://yandex.ru/maps/80/yuzhno-sakhalinsk/search/%D0%BF%D1%83%D0%BD%D0%BA%D1%82%D1%8B%20%D1%8D%D0%BA%D1%81%D1%82%D1%80%D0%B5%D0%BD%D0%BD%D0%BE%D0%B9%20%D1%81%D0%B2%D1%8F%D0%B7%D0%B8/"
# =====================================================
...
COMMAND_WORDS = [
    ...
    'доброволец', 'волонтер', 'волонтёр', 'поисковик', 'поиск людей', 'курсы', 'стать добровольцем', 'я доброволец',
    'пункты связи', 'пункты экстренной связи', 'экстренная связь', 'терминалы 112', 'где позвонить 112', '112',
    'дтп', 'авария', 'дорожная авария', 'попал в дтп', 'свидетель дтп', 'столкновение машин',
]
...
ROUTES = {
    ...
    'moskovskaya': {'name': 'Гора Московская', 'cat': 'mountains', 'lat': 46.917759, 'lon': 142.796927,
                'info': '572 м, Сусунайский хребет, популярная видовая площадка рядом с Южно-Сахалинском'},
    'lyagushka': {'name': 'Останец Лягушка (Весточка)', 'cat': 'mountains', 'lat': 46.8689, 'lon': 142.8888,
...
def handle_command(chat_id, command, message_id=None):
    ...
    if command in ('start', 'main', '/start', 'главное меню', 'привет', 'здравствуй', 'здравствуйте', 'добрый день', 'hello', 'hi'):
        ...
        send_message(chat_id,
            ...
            "5. Обработайте место разлива раствором хлорной извести либо горячим мыльно-содовым раствором (30 г соды + 40 г тёртого мыла на 1 л воды)\n"
        ...
        send_message(chat_id,
            "**⚡ Экстренные номера:**\n"
            "☎️ `112` — единый номер вызова экстренных служб\n"
            "🔥 `101` — пожарные\n"
            "👮 `102` — полиция\n"
            "🚑 `103` — скорая помощь\n"
            "💨 `104` — газовая служба\n\n"
            "**📍 Специализированные службы:**\n"
            "⛑️ [8 (4242) 240-304](tel:+74242240304) — поисково-спасательный отряд МЧС\n"
            "🔍 [8 (984) 184-10-10](tel:+79841841010) — ПСО «СОВА»\n\n"
            "**📍 Пункты экстренной связи города** — терминалы для вызова 112 на карте:",
            [
                [btn_link("🗺️ Пункты на Яндекс.Картах", SOS_POINTS_URL)],
                [btn("🏠 Главное меню", 'main')],
            ])
        ...
        send_message(chat_id,
            ...
            "• Давайте пить воду.\n\n"
            "❌ **НЕЛЬЗЯ:** использовать масло, жир, сметану, кремы; лёд; прокалывать пузыри; отдирать прилипшую одежду.\n\n"
    ...
    elif command in ['volunteer', 'доброволец', 'волонтер', 'волонтёр', 'поисковик', 'поиск людей', 'курсы', 'стать добровольцем', 'я доброволец']:
        send_message(chat_id,
            "**🙋‍♂️ Я — ДОБРОВОЛЕЦ!**\n\n"
            "_Научим искать пропавших людей и спасать жизни._\n\n"
            "Учебно-методический центр по гражданской обороне, чрезвычайным ситуациям "
            "и пожарной безопасности Сахалинской области набирает слушателей на программу "
            "**«Подготовка добровольцев (волонтёров) поиску пропавших людей»**.\n\n"
            "**В программе:**\n"
            "• порядок работ по поиску и взаимодействие с правоохранительными органами;\n"
            "• поведение в лесу, ориентирование, работа с картой и связью;\n"
            "• навыки оказания первой помощи;\n"
            "• психология работы с пострадавшими;\n"
    ...
    elif command in ['sos_points', 'пункты связи', 'пункты экстренной связи', 'экстренная связь', 'терминалы 112', 'где позвонить 112']:
        send_message(chat_id,
            "**📍 ПУНКТЫ ЭКСТРЕННОЙ СВЯЗИ**\n"
            "Специальные уличные терминалы в Южно-Сахалинске для вызова экстренных служб. "
            "Выручат, если телефон разряжен, сломан или нет связи.\n\n"
            "🗺️ Все точки — на Яндекс.Картах: нажмите кнопку ниже, и карта покажет "
            "ближайший пункт с адресом и маршрутом от вашего местоположения.\n\n"
            "💡 Список на карте обновляется автоматически — бот всегда ведёт на актуальные точки.\n\n"
            "☎️ Напомню: единый номер вызова экстренных служб — `112`.\n\n"
            "**📋 ЧТО СООБЩИТЬ ДИСПЕТЧЕРУ (5 пунктов):**\n"
            "1. **ЧТО случилось** — пожар, ДТП, человеку плохо, преступление.\n"
            "2. **ГДЕ** — точный адрес или ориентиры (улица, дом, километр трассы, заметные здания).\n"
            "3. **СКОЛЬКО пострадавших** и их состояние: в сознании ли, дышат ли, есть ли кровотечение.\n"
            "4. **КТО звонит** — фамилия, имя, контактный номер.\n"
            "5. **НЕ ВЕШАЙТЕ ТРУБКУ ПЕРВЫМ** — диспетчер уточнит детали и подскажет, что делать до приезда служб.\n\n"
            "⚠️ Говорите спокойно и коротко: диспетчеру нужны факты, а не подробности.\n"
            "📞 Позвонили по ошибке — не бросайте трубку, объясните, что всё в порядке, иначе вам перезвонят для проверки.",
            [
                [btn_link("🗺️ Пункты на Яндекс.Картах", SOS_POINTS_URL)],
                [btn("🏠 Главное меню", 'main')],
            ])

    elif command in ['dtp', 'дтп', 'авария', 'дорожная авария', 'попал в дтп', 'свидетель дтп', 'столкновение машин']:
        send_message(chat_id,
            "**🚗 ДТП: КАК ДЕЙСТВОВАТЬ**\n"
            "_По рекомендациям МЧС России._\n\n"
            "**🚙 Если вы попали в ДТП:**\n"
            "1. Остановитесь, включите аварийную сигнализацию, выставьте знак аварийной остановки.\n"
            "2. Проверьте, нет возгорания и запаха бензина. **Машина горит — немедленно покиньте салон** и помогите выбраться остальным.\n"
            "3. Выключите зажигание и отключите аккумулятор (**сначала минусовой провод**) — это исключит пожар и случайное срабатывание подушек.\n"
            "4. Позвоните `112` и сообщите: где произошло (адрес, километр трассы, ориентиры), сколько машин, есть ли пострадавшие, дополнительные опасности (утечка топлива, повреждённые опоры ЛЭП, столбы).\n"
            "5. При необходимости окажите первую помощь (раздел «🩺 Первая помощь»).\n\n"
            "**👀 Если вы стали свидетелем ДТП с пострадавшими:**\n"
            "1. Позвоните `112`.\n"
            "2. Обозначьте место: аварийка, знак остановки — чтобы не произошло новое ДТП.\n"
            "3. Нет возгорания — **не спешите вытаскивать людей из салона**. Если есть возможность подождать, оставьте извлечение спасателям: неосторожное извлечение усугубляет травмы.\n"
            "4. Подходите к машине осторожно: осколки стёкол, отлетевшие детали.\n"
            "5. **Пострадавший зажат в салоне и в сознании:** говорите с ним, поддерживайте контакт до прибытия бригады; следите за кровотечением и повязками; наблюдайте за обстановкой вокруг.\n"
            "6. **Без сознания:** периодически проверяйте дыхание и пульс.\n"
            "7. Подозрение на травму позвоночника или шеи — **не перемещайте пострадавшего** до прибытия спасателей.\n"
            "8. Эвакуируйте в первую очередь людей с наименьшими повреждениями; остальным оказывайте первую помощь до приезда бригад. **Аптечка есть в любом автомобиле.**\n\n"
            "**🔥 Немедленно извлекайте из салона — только если:**\n"
            "• возгорание, задымление, стойкий запах бензина;\n"
            "• нет дыхания или сердцебиения;\n"
            "• оказать помощь на месте невозможно.\n"
            "**Как извлечь пострадавшего в сознании:** проведите руки под его подмышки, зафиксируйте предплечья и осторожно извлеките наружу. Помните: возможны сложные переломы.\n\n"
            "🚑 Скорая — `103`, единый номер — `112`.",
            back_menu())
